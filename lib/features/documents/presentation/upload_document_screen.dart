...
class _UploadDocumentViewState extends State<_UploadDocumentView> {
  ...
  void didChangeDependencies() {
    ...
    if (!_initialized) {
      ...
      if (args != null && args is Map<String, dynamic>) {
        ...
        final orgType = args['organizationType'] as String?;
        final orgName = args['organizationName'] as String?;
        ...
        if (eventId != null &&
            ...
            orgName != null &&
            eventTitle != null) {
          final controller = context.read<DocumentUploadController>();
          WidgetsBinding.instance.addPostFrameCallback((_) {
            controller.initWithEventContext(
              eventId: eventId,
              organizationType: orgType,
              organizationName: orgName,
              eventTitle: eventTitle,
            );
            _titleController.text = eventTitle;
          });
        }
      }
    }
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  // Organization type toggle — hidden when locked
                  if (!controller.hasEventContext) ...[
                    _buildOrganizationTypeToggle(controller, l10n),
                    const SizedBox(height: 20),
                  ],
                  // Organization name — read-only display or dropdown
                  _buildOrganizationNameField(controller, l10n),
                  const SizedBox(height: 20),
                  // Title — read-only display or text field
  ...
  Widget _buildOrganizationTypeToggle(
      DocumentUploadController controller, AppLocalizations l10n) {
  ...
  // Shows a locked read-only field when event context exists,
      ...
      children: [
        ...
        if (controller.hasEventContext)
          _lockedField(
            ...
            value:
                '${controller.lockedOrganizationType} · ${controller.lockedOrganizationName}',
          )
        else
  ...
  Widget _buildOrganizationNameDropdown(
      DocumentUploadController controller, AppLocalizations l10n) {
  ...
  // Shows a locked read-only field when event context exists,
  // otherwise shows the text field.
      ...
      children: [
        ...
        if (controller.hasEventContext)
          _lockedField(
            ...
            value: controller.lockedEventTitle ?? '',
          )
        else
          TextField(
        ...
        GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            child: controller.hasFile
                ? _buildFilePreview(controller, l10n)
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: controller.isLoading
            ? null
            : () => _handleSubmit(context, controller, l10n),
  ...
  Future<void> _handleSubmit(BuildContext context,
      DocumentUploadController controller, AppLocalizations l10n) async {
    final success = await controller.submit();
    if (!context.mounted) return;
    if (success) _showSuccessDialog(context, controller, l10n);
  }
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text(
              controller.hasEventContext
                  ? l10n.paperworkSubmitted
                  : l10n.documentSubmitted,
              style: const TextStyle(
            ...
            Text(
              controller.hasEventContext
                  ? l10n.paperworkSubmittedMessage
                  : l10n.documentSubmittedMessage,
              textAlign: TextAlign.center,
  ...
  Widget _lockedField({required IconData icon, required String value}) {
    return Container(
